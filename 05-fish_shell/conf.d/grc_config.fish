...
set -U grc_plugin_execs ant blkid cvs diff docker docker-machine env id ip iostat \
	gcc g++ ifconfig make mount mtr netstat tail traceroute \
       wdiff        last lsattr lsblk lspci lsmod lsof getfacl getsebool uptime nmap \
...
for executable in $grc_plugin_execs
    if type -q $executable
        alias $executable "grc --colour=on $executable"
    end
end
